...
export default function CameraView({
  ...
  const pickPreferredCameraId = (cams, prev) => {
    ...
    const isAvoided = (cam) => {
      const label = String(cam?.label || "");
      return AVOID_CAMERA_MATCH.some((rx) => rx.test(label));
    };
    const normalCams = cams.filter((cam) => !isAvoided(cam));
    ...
    if (prev && pickPool.some((c) => c.deviceId === prev)) return prev;
    return pickPool[0].deviceId;
  };
